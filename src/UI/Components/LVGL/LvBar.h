...
namespace UI
{
    #define LV_BAR_DEFAULT_ANIM LV_ANIM_ON

    class LvBar : public LvObj
    {
      public:
		LvBar(const std::string& name, LvObj& parent);

		void setValue(int32_t value, lv_anim_enable_t anim = LV_BAR_DEFAULT_ANIM);
        void setStartValue(int32_t value, lv_anim_enable_t anim = LV_BAR_DEFAULT_ANIM);
